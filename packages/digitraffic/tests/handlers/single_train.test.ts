...
it('throws if train number is not a number', async () => {
  ...
  await expect(fn).rejects.and.toThrow(/Expected train number to be a number/)
})
...
it('throws if version is not a number or a string', async () => {
  ...
  await expect(fn).rejects.and.toThrow(
    /Expected version to be a number or a string/,
  )
})
...
it('throws if date is not in iso 8601', async () => {
  ...
  await expect(fn).rejects.and.toThrow(/Date didn't match RegExp/)
})
