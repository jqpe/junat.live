...
it('throws if station shortcode is not a string', async () => {
  // @ts-expect-error TypeScript should error if stationShortCode is not a string.
  await expect(() => fetchLiveTrains(null)).rejects.and.toThrowError(
    /Expected stationShortCode to be a string/,
  )
})

it('throws an error if rate limit is achieved', async () => {
  server.use(
    http.get(url, ({}) => {
  ...
  await expect(() => fetchLiveTrains('HKI')).rejects.and.toThrowError(
    DigitrafficError,
  )
})
