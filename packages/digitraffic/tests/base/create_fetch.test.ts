...
it("throws if path doesn't start with a slash /", async () => {
  await expect(async () => await createFetch('')).rejects.and.toThrow(TypeError)
})
...
it("throws an error if json couldn't be parsed", async () => {
  server.use(
  ...
  await expect(() => createFetch('/metadata/stations')).rejects.and.toThrow(
    SyntaxError,
  )
})
